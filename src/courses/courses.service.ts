...
export class CoursesService {
  ...
  async create(createCourseDto: CreateCourseDto, trainerId: string): Promise<Course> {
  ...
  async findAll(userId?: string, role?: string): Promise<Course[]> {
    ...
    if (role === 'trainer' && userId) {
      return this.courseModel.find({ trainerId: new Types.ObjectId(userId) }).exec();
    }
  ...
  async update(id: string, updateCourseDto: UpdateCourseDto, trainerId: string): Promise<Course> {
  ...
  async togglePublish(id: string, published: boolean, trainerId: string): Promise<Course> {
