...
export class EnrollmentSeeder {
  ...
  async seed() {
    ...
    if (learners.length === 0 || courses.length === 0) {
      console.log(
        '⚠️  No learners or courses found. Please seed users and courses first.',
      );
      return;
    }

    // Alice enrolls in Web Development - completed first module
    const webDevCourse = courses.find((c) =>
      c.title.includes('Web Development'),
    );
    if (webDevCourse) {
      const modules = await this.moduleModel
        .find({ courseId: webDevCourse._id })
        .sort({ order: 1 });
    ...
    const dataScienceCourse = courses.find((c) =>
      c.title.includes('Data Science'),
    );
