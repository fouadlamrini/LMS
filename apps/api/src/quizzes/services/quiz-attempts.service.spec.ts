...
describe('QuizAttemptsService', () => {
  ...
  describe('startAttempt', () => {
    it('FAIL: should throw if quiz is not published', async () => {
      jest
        ...
        .mockResolvedValue({ status: QuizStatus.DRAFT } as any);

      await expect(
        service.startAttempt(mockQuizId, mockLearnerId),
  ...
  describe('answerQuestion', () => {
    it('FAIL: should throw Forbidden if learner does not own the attempt', async () => {
      ...
      await expect(
        service.answerQuestion('some-id', {} as any, mockLearnerId),
